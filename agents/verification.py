...
GRADE_SCHEMA = {
    ...
    "json_schema": {
        ...
        "schema": {
            "type": "object",
            # ★ 필드 순서가 설계값이다 (2026-07-29, 결함 M). 구조화 출력은 스키마
            # 순서대로 토큰을 생성하므로, score 가 앞에 있으면 모델은 **판단 근거를
            # 쓰기 전에 점수를 먼저** 내놓아야 한다. Router에서 같은 구조를 고쳐
            # 88.6% → 99.0~99.5% 가 됐다(결함 J·K, premature serialization,
            # arXiv:2606.09410).
            #
            # 2×2 요인 실험(eval/schema_order.py, 30건 × 3회 × 3실행) 결과:
            #   채택률이 사람 라벨(56.7%)에 접근  — 순서 주효과 +13.7%p
            #   실행 간 흔들림이 11.1%p → 3.3%p 로 감소 (재현성 개선)
            # κ·정밀도·재현율은 실행 간 흔들림이 조건 간 차이보다 커서 판정하지 못했다.
            # 즉 이 변경의 확인된 효과는 **정확도가 아니라 채택 기준과 재현성**이다.
            #
            # required 순서도 함께 바꿔야 한다 — Router에서 required 가 생성 순서에
            # 영향을 주는 것이 확인됐다.
            "properties": {
                ...
                "score": {
                    ...
                    "description": "1(전혀 아님)~5(명백히 맞음) 척도의 채점 점수. "
                                   "위 reasoning 에서 원문과 대조한 결과를 근거로 정한다.",
                },
            },
...
- 4점: 주제와 지역·대상 중 **한쪽은 정확히 부합**하고 다른 한쪽이 인접 범위인 경우
  (예: 주제는 정확히 연구대상인데 지역이 인접 권역, 또는 지역은 목표시장인데
   주제가 인접 카테고리)
- 3점: 부분적으로만 관련(예: 인접 카테고리, 인접 지역, 지역 불명)
- 2점: 주제와 지역·대상 중 **한쪽이 완전히 범위 밖**인 경우 (다른 시장에 대한 서술,
  또는 전혀 다른 카테고리인데 지역만 맞는 경우)
- 1점: 전혀 다른 주제/지역/카테고리""",
...
- 4점: 수치·날짜·고유명사가 모두 원문과 일치하고 원문에 없는 내용을 덧붙이지도
  않았으나, **원문이 달아 둔 조건·범위·시점을 fact가 빠뜨려** 원문보다 단정적으로
  읽히는 경우 (원문 "북미 한정 34%" → fact "34%")
